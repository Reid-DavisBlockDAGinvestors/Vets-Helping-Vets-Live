  ...
  try {
    switch (event.type) {
      case 'payment_intent.succeeded': {
        ...
        console.log('[webhook] one-time success', pi.id, pi.metadata)
      ...
      case 'charge.succeeded': {
        ...
        console.log('[webhook] charge succeeded', ch.id)
        await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/receipt/global`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
      ...
      case 'checkout.session.completed': {
        ...
        console.log('[webhook] checkout session completed', cs.id, cs.mode)
      ...
      case 'invoice.paid': {
        ...
        console.log('[webhook] subscription paid', inv.id, inv.subscription)
