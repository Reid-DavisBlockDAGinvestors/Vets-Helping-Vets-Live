...
/**
 * POST /api/admin/tokens/soulbound
 * Make a token soulbound (non-transferable) or remove soulbound status
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const { tokenId, soulbound, chainId, contractVersion } = body

    if (tokenId === undefined) {
      return NextResponse.json({ error: 'tokenId required' }, { status: 400 })
    }
    if (soulbound === undefined) {
      return NextResponse.json({ error: 'soulbound (true/false) required' }, { status: 400 })
    }
    ...
    try {
      logger.info(`[admin/tokens/soulbound] ${soulbound ? 'Making' : 'Removing'} soulbound for token ${tokenId}`)
      const tx = soulbound
        ? await contract.makeSoulbound(BigInt(tokenId))
        : await contract.removeSoulbound(BigInt(tokenId))
      ...
      logger.error(`[admin/tokens/soulbound] Error: ${e?.message}`)
      return NextResponse.json({
        error: `Failed to ${soulbound ? 'make' : 'remove'} soulbound`,
        details: e?.message
      }, { status: 500 })
    }

  } catch (e: any) {
    logger.error('[admin/tokens/soulbound] Error:', e)
