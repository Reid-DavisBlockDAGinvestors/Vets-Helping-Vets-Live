...
export async function POST(req: NextRequest) {
  try {
    ...
    const { pause, chainId, contractVersion } = body

    if (pause === undefined) {
      return NextResponse.json({ error: 'pause (true/false) required' }, { status: 400 })
    }
    ...
    try {
      if (pause) {
        logger.info(`[security/pause] Pausing contract on chain ${targetChainId}`)
        ...
        logger.info(`[security/pause] Unpausing contract on chain ${targetChainId}`)
      ...
      logger.error(`[security/pause] Error: ${e?.message}`)
      return NextResponse.json({
        error: `Failed to ${pause ? 'pause' : 'unpause'} contract`,
        details: e?.message
      }, { status: 500 })
    }

  } catch (e: any) {
    logger.error('[admin/security/pause] Error:', e)
